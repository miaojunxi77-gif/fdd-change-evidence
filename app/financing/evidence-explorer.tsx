...
const conflictReasons=["Cross-source versions — chronology QC pending","Same-source multiple versions — chronology QC pending","No safe chronology / adjudication needed"];

function conflictStructure(r:BrandYear){
 if(r.score!==null)return "";
 const src=[...new Set((r.sources||[]).filter(Boolean))];
 if(src.length>1)return "Cross-source versions — chronology QC pending";
 if(r.versions>1)return "Same-source multiple versions — chronology QC pending";
 return "No safe chronology / adjudication needed";
}
...
function conflictDetail(d:Detail){
 ...
 if(sources.length>1)return `Current V1.1 cross-source chronology review: versions from ${sources.join(" / ")} imply ${scoreText}. A source/state difference by itself is NOT a sufficient reason to keep the brand-year unresolved. If a later amendment, revision, or effective date unambiguously supersedes an earlier version for the same offering, the later version should control. This row remains NA in the current V1.1 freeze until that chronology is re-adjudicated.`;
 if(sources.length===1&&dated.length===versions.length&&dates.length===1)return `Same-source, same-date parallel-version conflict: ${sources[0]} contains ${versions.length} distinct Item 10 texts dated ${dates[0]} that imply ${scoreText}. This can arise from Original/Marked Copy or other parallel filed copies; because the recorded date does not order the versions, no controlling version is inferred automatically.`;
 if(sources.length===1&&dates.length>1)return `Same-source chronology review: ${sources[0]} contains distinct Item 10 versions with recorded dates ${dates.join(" / ")} that imply ${scoreText}. A clearly later amendment or superseding filing should control when the document metadata establishes that relationship; otherwise the row remains unresolved. Current V1.1 has not completed that amendment-level re-adjudication for this row.`;
 if(sources.length===1)return `Same-source multi-version chronology review: ${sources[0]} contains distinct Item 10 texts that imply ${scoreText}, but recorded dates/version ordering are incomplete or ambiguous in the current website data. The score remains NA pending chronology review rather than being forced.`;
 return `Multiple valid Item 10 versions imply ${scoreText}. The row remains NA until chronology or document-level adjudication establishes a controlling version.`;
}
...
function InlineBrandDetail({row,detail,loading,onClose}:{row:BrandYear;detail:Detail|null;loading:boolean;onClose:()=>void}){
 return <div className="financing-inline-detail">
  ...
  {!loading&&detail&&<>
   {detail.score===null&&<div className="financing-conflict-explanation"><strong>Current V1.1 QC status</strong><p>{conflictDetail(detail)}</p><p className="financing-conflict-instruction">Each card below is a separate Item 10 version. Compare the <b>source/state, recorded filing date, score, and score-specific evidence</b> directly. Amendment/revision dates embedded in the FDD may require an additional chronology pass.</p></div>}
...
export default function EvidenceExplorer({basePath}:{basePath:string}){
 ...
 return <div className="financing-explorer">
  ...
  <div className={`financing-filters${mode==="brand"&&status==="unresolved"?" has-conflict-filter":""}`}><label className="financing-search"><span>Search brand, form, provider or text</span><input value={query} onChange={e=>reset(()=>setQuery(e.target.value))} placeholder="e.g., McDonald's, guarantee, deferred fee"/></label><label><span>Year</span><select value={year} onChange={e=>reset(()=>setYear(e.target.value))}><option value="all">All years</option>{years.map(y=><option key={y}>{y}</option>)}</select></label>{mode==="brand"?<><label><span>Risk score</span><select value={score} onChange={e=>reset(()=>setScore(e.target.value))}><option value="all">All scores</option>{[0,1,2,3,4].map(s=><option key={s} value={s}>Score {s}</option>)}<option value="na">NA / conflict</option></select></label><label title="Unresolved means the current V1.1 system-year has multiple Item 10 versions that imply different scores and has not yet been given a safe controlling-version rule."><span>Status</span><select value={status} onChange={e=>reset(()=>{setStatus(e.target.value);if(e.target.value!=="unresolved")setConflictReason("all")})}><option value="paper">Paper-ready</option><option value="unresolved">Unresolved score conflict</option><option value="all">All observations</option></select></label>{status==="unresolved"&&<label title="These are current V1.1 triage structures, not final economic reasons. Later amendments should control when chronology is unambiguous."><span>Conflict / QC structure</span><select value={conflictReason} onChange={e=>reset(()=>setConflictReason(e.target.value))}><option value="all">All current V1.1 conflicts (155)</option>{conflictReasons.map(reason=><option key={reason} value={reason}>{reason} ({conflictCounts[reason]||0})</option>)}</select></label>}</>:<label><span>Change type</span><select value={kind} onChange={e=>reset(()=>setKind(e.target.value))}><option value="all">All score changes</option><option value="0 → support">0 → support</option><option value="support → 0">Support → 0</option><option value="other increase">Other increase</option><option value="other decrease">Other decrease</option></select></label>}</div>
  <div className="financing-result-line"><strong>{active.length.toLocaleString()}</strong> matching {mode==="brand"?"brand-year observations":"consecutive-year score changes"}<span>{mode==="brand"&&status==="unresolved"?(conflictReason==="all"?"Current V1.1 freeze: 155 conflicts. Cross-source and same-source labels are chronology-QC triage flags; source difference alone does not make a case unresolved.":`Current V1.1 QC structure: ${conflictReason}. Click Compare versions to inspect each source/state, recorded date, score and Item 10 excerpt.`):"Click a row to inspect the original extracted text."}</span></div>
  {loading&&<div className="financing-loading">Loading evidence dataset…</div>}{error&&<div className="financing-error">{error}</div>}
  {!loading&&!error&&mode==="brand"&&<div className="financing-table-wrap"><table className="financing-evidence-table"><thead><tr><th>Franchise system</th><th>Year</th><th>Score / conflict QC</th><th>Form / provider</th><th>{status==="unresolved"?"Conflicting sources":"Evidence excerpt"}</th><th/></tr></thead><tbody>{(visible as BrandYear[]).map(r=><Fragment key={r.id}><tr className={selected?.id===r.id?"selected":""}><td><strong>{displayBrand(r.chainId,r.brand)}</strong><small>{r.chainId} · {r.documents} document{r.documents===1?"":"s"}</small></td><td>{r.year}</td><td><Badge score={r.score}/><small>{r.score===null?conflictStructure(r):scoreLabels[r.score]}</small></td><td><strong>{joined(r.forms)}</strong><small>{joined(r.providers)}</small></td><td>{r.score===null?<div className="financing-source-preview"><strong>{r.sources.length>1?"Sources / state sources to compare":"Source with multiple versions"}</strong><div>{r.sources.map(s=><span key={s}>{s}</span>)}</div><small>Source difference is only a triage flag. Amendment/effective chronology should determine the controlling version when it is unambiguous.</small></div>:<q>{r.evidenceQuote||"No short evidence quote recorded."}</q>}</td><td><button onClick={()=>openRow(r)}>{selected?.id===r.id?"Close":r.score===null?"Compare versions":"View original"}</button></td></tr>{selected?.id===r.id&&<tr className="financing-inline-detail-row"><td colSpan={6}><InlineBrandDetail row={r} detail={detail} loading={detailLoading} onClose={()=>{setSelected(null);setDetail(null)}}/></td></tr>}</Fragment>)}</tbody></table></div>}
