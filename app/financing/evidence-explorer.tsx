...
export default function EvidenceExplorer({basePath}:{basePath:string}){
 ...
 return <div className="financing-explorer">
  ...
  <div className="financing-result-line"><strong>{active.length.toLocaleString()}</strong> matching {mode==="brand"?"brand-year observations":"consecutive-year score changes"}<span>{mode==="brand"&&status==="unresolved"?(conflictReason==="all"?"155 total: 93 prior V1 conflicts retained + 62 newly surfaced after identity repair. Each row labels the observable version structure; open it for sources, dates and score-specific text.":`Conflict reason: ${conflictReason}. Open a row for sources, dates and score-specific text.`):"Click a row to inspect the original extracted text."}</span></div>
