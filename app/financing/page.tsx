...
export default function FinancingPage() {
  ...
  return <main>
    <section className="page-hero financing-hero shell">
      ...
      <div className="financing-hero-metrics" aria-label="Item 10 production V1.2 sample"><div><strong>13,237</strong><span>reconciled brand-years</span></div><div><strong>13,124</strong><span>paper-ready observations</span></div><div><strong>99.15%</strong><span>paper-ready coverage</span></div><div><strong>113</strong><span>unresolved after V1.2 QC</span></div></div>
    </section>
    ...
    <section className="financing-section financing-paper-section"><div className="shell"><div className="financing-section-heading compact"><div><p className="eyebrow">RISK-BEARING LADDER</p><h2>One label is not enough for “financing support”</h2></div><p>Score 1 主要降低搜索或接入成本；Score 2–4 才逐步增加 franchisor/affiliate 的信用暴露。</p></div><div className="financing-score-layout"><div className="financing-score-ladder">{scoreDistribution.map(row=><article key={row.score}><span className="financing-score-number" style={{borderColor:row.color,color:row.color}}>{row.score}</span><div><strong>{row.label}</strong><p>{row.n.toLocaleString()} brand-years · {row.share.toFixed(2)}%</p></div></article>)}</div><div className="financing-distribution-card"><p className="eyebrow">SCORE DISTRIBUTION</p><h3>Paper-ready sample, N = 13,124</h3><div className="financing-bars">{scoreDistribution.map(row=><div key={row.score}><span>Score {row.score}</span><div><i style={{width:`${row.share}%`,background:row.color} as CSSProperties}/></div><strong>{row.share.toFixed(2)}%</strong></div>)}</div><p className="financing-chart-note">Score 0 仍约占三分之二；“Any support”不能等同于直接贷款。</p></div></div></div></section>

    <section className="financing-section shell"><div className="financing-section-heading compact"><div><p className="eyebrow">HEADLINE PREVALENCE</p><h2>Support is common; direct funded credit is not</h2></div><p>以下比例均以 13,124 个 V1.2 paper-ready brand-years 为分母。</p></div><div className="financing-threshold-grid"><article><span>Score ≥ 1</span><strong>33.41%</strong><h3>Any support</h3><p>4,385 brand-years。</p></article><article><span>Score ≥ 2</span><strong>27.58%</strong><h3>Risk-bearing support</h3><p>3,620 brand-years。</p></article><article><span>Score ≥ 3</span><strong>14.35%</strong><h3>Material credit risk</h3><p>1,883 brand-years。</p></article><article><span>Score = 4</span><strong>11.53%</strong><h3>Direct funded credit</h3><p>1,513 brand-years。</p></article></div></section>

    <section className="financing-section shell"><div className="financing-section-heading compact"><div><p className="eyebrow">TIME TREND</p><h2>Annual prevalence by financing-risk threshold</h2></div><p>横轴现在明确保留 2009。该年 <b>n = 0</b>，因此显示为缺口而不是 0%：没有 paper-ready Item 10 observation，就没有可定义的 prevalence。2008–2012 年样本极小，只用于展示 coverage。</p></div><TrendChart/><div className="financing-year-samples" aria-label="Annual sample sizes">{annual.map(row=><span key={row.year}><strong>{row.year}</strong>{row.n===0?"n=0 · no obs":`n=${row.n.toLocaleString()}`}</span>)}</div></section>

    <section className="financing-section financing-change-section"><div className="shell"><div className="financing-section-heading compact"><div><p className="eyebrow">WITHIN-FIRM CHANGES</p><h2>The direct answer to “How many changes?”</h2></div><p>分母是同一 reconciled offering/system 的 7,070 个 consecutive paper-ready brand-year pairs。</p></div><div className="financing-change-hero"><article><span>Intensity definition</span><strong>349</strong><h3>RiskScore changes</h3><p>349 / 7,070 = <b>4.94%</b>。</p></article><article><span>Binary definition</span><strong>281</strong><h3>No support ↔ any support</h3><p>281 / 7,070 = <b>3.97%</b>：135 次开始提供，146 次停止提供。</p></article></div><div className="financing-transition-grid"><article><strong>135</strong><span>0 → support</span><small>1.91% of pairs</small></article><article><strong>146</strong><span>support → 0</span><small>2.07% of pairs</small></article><article><strong>168</strong><span>risk increases</span><small>2.38% of pairs</small></article><article><strong>181</strong><span>risk decreases</span><small>2.56% of pairs</small></article><article><strong>63</strong><span>0–2 → 3–4</span><small>0.89% of pairs</small></article><article><strong>47</strong><span>3–4 → 0–2</span><small>0.66% of pairs</small></article></div></div></section>

    <section className="financing-section shell" id="financing-evidence"><div className="financing-section-heading compact"><div><p className="eyebrow">AUDITABLE EVIDENCE EXPLORER</p><h2>Firm, year, score—and the original Item 10 language</h2></div><p>检索 V1.2 的 13,237 个 reconciled brand-years，或比较 349 个 within-firm score changes。Unresolved filter 现在使用 V1.2 QC 的真实原因，而不是简单按 source 数量推断。</p></div><div className="financing-evidence-notes"><article><strong>10,886 / 13,237</strong><span>brand-years with full extracted text</span><p>其余记录仍展示用于 coding 的 evidence quote；原始 PDF 始终是最终权威来源。</p></article><article><strong>19</strong><span>chronology resolutions</span><p>例如 1 Percent Lists 2025 已按 September amendment 取 Score 2，不再属于 unresolved。</p></article><article><strong>113</strong><span>remaining unresolved</span><p>网页可按 state-specific、parallel version、metadata insufficient 等 V1.2 QC reason 筛选。</p></article></div><EvidenceExplorer basePath={basePath}/></section>

    <section className="financing-section shell"><div className="financing-section-heading compact"><div><p className="eyebrow">CONSERVATIVE V1.2 FREEZE</p><h2>Remaining unresolved cases stay missing, not forced into a score</h2></div><p>113 observations（0.85%）在 chronology / identity / offering-scope QC 后仍保留为 NA。</p></div><div className="financing-sensitivity-grid">{[["Any support","33.13%","33.98%"],["Risk-bearing","27.35%","28.20%"],["Material risk","14.23%","15.08%"],["Direct funded","11.43%","12.28%"]].map(([label,low,high])=><article key={label}><h3>{label}</h3><div><span>{low}</span><i/><span>{high}</span></div><p>lower–upper bound</p></article>)}</div><p className="financing-sensitivity-note">V1.2 unresolved 的最大 bound width 约 0.85 percentage points；主要 prevalence 结论不由这些 cases 驱动。</p></section>

    <section className="financing-section financing-download-section"><div className="shell financing-download-inner"><div><p className="eyebrow">PRODUCTION V1.2 · FROZEN 2026-09-08</p><h2>Chronology / identity QC rebuild</h2><p>本页已切换到 V1.2。完整 Advisor Package V1.2 与 release ZIP 为本次 QC 后的正式可交付版本；网页保留原始版本对照与逐条 unresolved reason 供核查。</p></div></div></section>
  </main>;
}
